...
CSV): **Render** (render.com — easiest, connect a GitHub repo, it detects
Flask automatically; add `gunicorn app:app` as the start command) or
**Railway** (railway.app). Both have free/low-cost tiers sufficient for a
lead-gen tool at low volume. Set `ANTHROPIC_API_KEY` and `SECRET_KEY` as
